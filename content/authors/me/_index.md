---
title: Milad Shariatmadari

role: PhD Researcher in Linguistics

bio: >
  I am a linguist specializing in language variation and change from a
  usage-based perspective. My research combines quantitative linguistics,
  corpus studies, statistical modeling, and experimental approaches.
  My main research interests include word order variation, complex
  predicates, grammaticalization, and the languages of West Asia and
  the Caucasus.

organizations:
  - name: LATTICE (CNRS / ENS-PSL / Sorbonne Nouvelle)
    url: https://www.lattice.cnrs.fr/membres/anciens-membres/milad-shariatmadari/

interests:
  - Word Order Variation
  - Syntax–Semantics Interface
  - Areal Typology and Language Contact
  - Language Production and Processing
  - Grammaticalization

education:
  courses:
    - course: PhD in Linguistics
      institution: Sorbonne Nouvelle University
      year: 2025

    - course: MA in Linguistics
      institution: Allameh Tabataba'i University
      year: 2017

    - course: BA in English Translation and Interpretation
      institution: Zand Institute of Higher Education
      year: 2014

social:
  - icon: envelope
    icon_pack: fas
    link: mailto:milad.shariatmadari@sorbonne-nouvelle.fr

  - icon: google-scholar
    icon_pack: ai
    link: https://scholar.google.com/citations?user=EEaKTdUAAAAJ&hl=en&oi=ao

  - icon: orcid
    icon_pack: ai
    link: https://orcid.org/0009-0006-6593-499X

  - icon: hal
    icon_pack: ai
    link: https://hal.science/search/index/q/*/authIdHal_s/miladshariatmadari

  - icon: github
    icon_pack: fab
    link: https://github.com/milad-shariatmadari

superuser: true
---